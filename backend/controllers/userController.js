...
const getProfile = async (req, res) => {
    try {
        ...
        const { id: userId } = req.user; //chatgpt
...
const updateProfile = async (req, res) => {
    try {
        ...
        const userId = req.user.id;  //chatgpt
...
const paymentStripe = async (req, res) => {
  try {
    ...
    console.log("Currency from ENV:", process.env.CURRENCY);
    console.log("Amount:", appointmentData.amount * 100);

    // Create a PaymentIntent in Stripe
    const paymentIntent = await stripe.paymentIntents.create({
      amount: Math.round(appointmentData.amount * 100), // in cents/paise
...
const verifyStripePayment = async (req, res) => {
  try {
    ...
    if (!paymentIntentId || !appointmentId) {
      return res.json({ success: false, message: "Missing paymentIntentId or appointmentId" });
    }
